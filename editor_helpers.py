...
# from assets_helpers import _inject_lora_simple
# from assets_helpers import _inject_lora_simple, handle_pose_generation, save_or_update_pose
# from assets_helpers import _inject_lora_simple, handle_pose_generation, save_or_update_pose, _resolve_asset_aux, save_uploaded_pose
...
def _build_sequence_assets_html(data: Dict, seq_id: str) -> str:
    ...
    #     color = "#f7e8a6" if has_selection else "#666"  # --color-vid from CSS
    #     return f"""
    ...
    #         if b64:
    #             return f"""
    #                 <div style='width:64px; height:64px; border-radius:4px; overflow:hidden; flex-shrink:0;'>
    #                     <img src='{b64}' style='width:100%; height:100%; object-fit:cover;' />
    #                 </div>
    #             """
    #     return """
    #         <div style='width:64px; height:64px; border:2px dashed #555; border-radius:4px; 
    ...
    #     has_selection = bool(vid_obj.get("selected_video_path"))
    #     icon = "▶" if has_selection else "▷"
    #     color = "#f7e8a6"  # --color-vid from CSS, same for both states
    #     return f"""
    #         <div style='display:flex; align-items:center; justify-content:center; 
    #                     width:20px; flex-shrink:0;'>
    #             <span style='color:{color}; font-size:14px;'>{icon}</span>
    ...
    # def _kf_thumbnail_html(kf_obj: dict) -> str:
    #     """Generate keyframe thumbnail or placeholder."""
    #     kf_color = "#d9a27c"  # --color-kf from CSS
    #     selected_path = kf_obj.get("selected_image_path")
    #     if selected_path and Path(selected_path).exists():
    #         b64 = _img_to_base64(selected_path)
    #         if b64:
    ...
    #                             border:2px solid {kf_color};'>
    #                     <img src='{b64}' style='width:100%; height:100%; object-fit:cover;' />
    #                 </div>
    #             """
    #     return f"""
    #         <div style='width:96px; height:96px; border:2px dashed {kf_color}; border-radius:4px; 
    #                     display:flex; align-items:center; justify-content:center; flex-shrink:0;
    #                     background:#2a2a2a;'>
    #             <span style='color:{kf_color}; font-size:24px;'>?</span>
    #         </div>
    #     """
    def _video_icon_html(vid_obj: dict) -> str:
        """Generate video status icon matching node selector style."""
...
def _duplicate_sequence(data: Dict[str, Any], seq_id: str) -> Tuple[Dict[str, Any], str]:
    ...
    new_seq["id"] = new_id
    # Note: no 'sequence_id' or 'order' - these are obsolete
    
    # 2. Calculate safe starting indices for Children (Global Scan)
    # We must scan ALL sequences to ensure global uniqueness for the new IDs
    max_kf_num = 0
    max_vid_num = 0
...
def _delete_keyframe(data: Dict[str, Any], seq_id: str, kf_id: str) -> Tuple[Dict[str, Any], str]:
    ...
    #     if kf_id in seq["keyframe_order"]:
    #         seq["keyframe_order"].remove(kf_id)
...
#         seq["video_plan"][which] = bool(value)
#         d = float(data["project"]["inbetween_generation"].get("duration_default_sec", 3.0))
#         _refresh_video_chain(seq, d, data)
#     return data
...
#     # Trace: Extract duration and project default for UI labels
#     val = node.get("duration_override_sec")
#     default_dur = int(float(data["project"]["inbetween_generation"].get("duration_default_sec", 3.0)))
    
#     # Radio value is the actual value or None
#     radio_value = str(int(float(val))) if val is not None else None
    
#     # Display logic for label and button (consistent with _eh_node_selected)
#     label_text = f"Length: {radio_value}s" if val else f"Clip Length (Inherited Default: {default_dur} seconds)"
...
#     # Trace: Compare restored value to current default. If they match, treat as Inherited.
...
#     if restored_val is not None and int(float(restored_val)) == default_dur:
...
# def _eh_generate_pose_for_keyframe(project_dict: dict, nid: str, kf_prompt: str):
#     """
#     Generates a pose using the keyframe's prompt, saves it, and sets it on the keyframe.
#     Yields updates for: kf_pose, kf_pose_preview, kf_pose_gallery, kf_cn_pose_thumb, kf_cn_shape_thumb, kf_cn_outline_thumb, status
#     """
#     # Get poses directory
#     poses_dir = get_project_poses_dir(project_dict)
#     if not poses_dir:
#         yield gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), "Error: Could not determine poses directory."
#         return
...
#     yield gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), "Generating pose..."
    
#     # Call handle_pose_generation (it's a generator)
#     for result in handle_pose_generation(prompt, project_dict, use_animal, char_count, mode):
#         # result is a 10-tuple: main, pose, shape, outline, log, json, temp_path, state_pose, state_shape, state_outline
#         if result and len(result) >= 10:
#             final_main_path = result[0]
#             final_pose_path = result[7]  # state_pose
#             final_shape_path = result[8]  # state_shape
#             final_outline_path = result[9]  # state_outline
#             status_log = result[4] or "Generating..."
            
#             # Stream status updates
#             yield gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), status_log
    
#     if not final_main_path:
#         yield gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), "Error: Pose generation failed."
#         return
    
#     # Save the pose with auto-generated name
#     pose_name = _sanitize_filename(prompt, fallback="generated_pose")
...
#     if "Error" in save_status:
#         yield gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), save_status
#         return
    
#     # Determine the saved pose path (with suffixes)
#     # The save function adds _1CHAR suffix for "1 Character"
#     saved_name = pose_name + "_1CHAR"
    
#     # Find the actual saved file (may have version number)
#     saved_pose_path = None
#     if poses_dir.exists():
#         for f in poses_dir.iterdir():
#             if f.is_file() and f.stem.startswith(saved_name) and f.suffix.lower() in {'.png', '.jpg', '.jpeg', '.webp'}:
#                 saved_pose_path = str(f)
#                 break
    
#     # If we couldn't find it, try to get the most recent file
#     if not saved_pose_path and poses_dir.exists():
#         img_files = sorted(
#             [f for f in poses_dir.iterdir() if f.is_file() and f.suffix.lower() in {'.png', '.jpg', '.jpeg', '.webp'}],
#             key=lambda x: x.stat().st_mtime,
...
#     if not saved_pose_path:
#         yield gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), "Error: Could not locate saved pose file."
#         return
    
#     # Resolve auxiliary images
#     from assets_helpers import _resolve_asset_aux
#     pose_thumb = _resolve_asset_aux(saved_pose_path, "poses")
#     shape_thumb = _resolve_asset_aux(saved_pose_path, "shapes")
#     outline_thumb = _resolve_asset_aux(saved_pose_path, "outlines")
...
#     # Final yield: set the pose path (which will trigger kf_pose.change and update everything)
#     yield (
#         gr.update(value=saved_pose_path),  # kf_pose
#         gr.update(value=saved_pose_path),  # kf_pose_preview
#         gr.update(value=gallery_items),     # kf_pose_gallery
...
#         # f"Pose saved: {Path(saved_pose_path).name}"  # status
#     )
...
# def _eh_set_selected_video(project_dict: dict, nid: str, selected_path: str):
#     # Return project_dict unchanged to avoid triggering full redraw
#     # Just update the video player with the new path
#     return project_dict, selected_path if selected_path else None

def _eh_next_kf_image(project_dict: dict, nid: str, current_path: str):
    data = project_dict
    image_list = _get_kf_gallery_images(data, nid)
    if not image_list: return gr.update()
    ...
    # Only update the dropdown; let the dropdown's change event handle the rest
    return gr.update(value=new_path)
...
def _eh_next_vid_clip(project_dict: dict, nid: str, current_path: str):
    ...
    return gr.update(choices=clip_list, value=clip_list[prev_index])
...
def _eh_prev_vid_clip(project_dict: dict, nid: str, current_path: str):
    ...
    return gr.update(choices=clip_list, value=clip_list[next_index])
...
def _eh_node_selected(project_dict: dict, raw_value, cur_sel: str):
    ...
    if kind == "vid":
        ...
        # Safe string conversion for Radio component
        # raw_dur = v.get("duration_override_sec")
        # dur_str = str(int(float(raw_dur))) if raw_dur is not None else None

        # vid_vals = [
        #     gr.update(value=s_path), gr.update(value=e_path), 
        #     gr.update(value=dur_str),
        #     gr.update(value=v.get("inbetween_prompt", "")), gr.update(value=v.get("negative_prompt", "")),
        #     gr.update(value="Default")
        # ]

        # Trace: Determine if we are using override or default
        # Trace: Determine if we are using override or default
        # Trace: Use the value if present, otherwise use project default
        # Trace: Use centralized helper for duration UI
        # Trace: Use centralized helper for duration UI
...
def _eh_run_and_curate_image(project_dict: dict, nid: str, count: int, path_at_start: str, current_selection: str):
    """
    ...
    for i in range(max(1, int(count))):
        # 1. GENERATION
        # gen = handle_test_generation(project_dict, nid, path_at_start)
        # result_path = None
        # for result in gen:
        #     result_path = result
        # 1. GENERATION
        ...
        if user_still_here and result_path:
            ...
            yield (
                ...
                gr.update(choices=choices_for_ui, value=val_norm),
                gr.update(value=val_norm),
                gr.update(value=val_norm),
                val_norm
            )
        elif result_path:
...
def _eh_run_and_curate_video(project_dict: dict, nid: str, count: int, path_at_start: str, current_selection: str):
    """
    ...
    for i in range(max(1, int(count))):
        ...
        # gen = handle_test_video_generation(project_dict, nid)
        # result_path = None
        # for result in gen:
        #     result_path = result
        # Consume generator to execute generation
        ...
        if user_still_here and result_path:
            ...
            yield (
                gr.update(interactive=False),
                f"Generated {i + 1}/{count}",
                gr.update(choices=choices_for_ui, value=val_norm),  # Use normalized value
                gr.update(value=val_norm),  # Show in player
                val_norm  # Result buffer
            )
        elif result_path:
            ...
            yield (
                ...
                gr.update(),  # Don't update player
                result_path   # Still pass to buffer
            )
        else:
            # No result
            yield (
...
def build_editor_tab(preview: gr.Code, settings_json: gr.State, current_file_path: gr.State, generation_result_buffer: gr.State, features: Dict = {}):
    ...
    with gr.Row():
        ...
        with gr.Column(scale=6):
                ...
                with gr.Row():
                    ...
                    with gr.Column(scale=1):
                        with gr.Accordion("Properties", open=True, elem_classes=["themed-accordion", "kf-theme"]) as kf_props:
                            # with gr.Row():
                            #     kf_pose_preview = gr.Image(label="Current Pose", visible=True, interactive=False, show_label=True, height=80, scale=1, show_download_button=False)
                            #     kf_pose = gr.Textbox(label="Pose Path", visible=False, interactive=True)
                            #     with gr.Column():
                            #         kf_clear_pose_btn = gr.Button("Clear Pose", size="sm", variant="secondary")
                            #         kf_generate_pose_btn = gr.Button("Generate Pose", size="sm", variant="secondary")
                            ...
                            with gr.Column(variant="panel"):
                                ...
                                with gr.Row():
                                    ...
                                    # message = gr.Markdown("Full Pose editor is found in Assets")
                                # message = gr.Markdown("<span style='font-size: 0.85em; font-style: italic; color: #999;'>Full Pose editor is found in Assets</span>")
                            # with gr.Row(equal_height=False):
                            #     kf_pose_preview = gr.Image(label="Current Pose", visible=True, interactive=False, show_label=True, height=140, scale=0, min_width=140, show_download_button=False)
                            #     kf_pose = gr.Textbox(label="Pose Path", visible=False, interactive=True)
                            #     with gr.Column(scale=1, min_width=120,elem_classes=["pose-buttons-col"]):
                            #         kf_clear_pose_btn = gr.Button("Clear",  variant="secondary")
                            #         kf_generate_pose_btn = gr.Button("Generate New",  variant="secondary")
                            #         kf_pose_upload_btn = gr.UploadButton(
                            #             "Upload", 
                            #             file_types=["image"], 
                            #             file_count="single",
                            ...
                            with gr.Row():
                                kf_char_left = gr.Dropdown([("", "")], value="", label="Character (primary/left)", info="Use this for 1CHAR poses/workflow", filterable=False, allow_custom_value=False)
                    ...
                    with gr.Column(scale=1):
                        with gr.Accordion("Generations", open=True, elem_classes=["themed-accordion", "kf-theme"]):
                            with gr.Group():
                                kf_generate_count = gr.Number(label="Iterations to Generate", info="Closing browser may disrupt this generation.", value=1, precision=0, minimum=1, interactive=True)
                                with gr.Row():
                ...
                with gr.Row():
                    ...
                    with gr.Column(scale=1):
                        with gr.Accordion("Properties", open=True, elem_classes=["themed-accordion", "vid-theme"]) as vid_props:
                            ...
                            with gr.Accordion("Advanced", open=False):
                                vid_lora = gr.Dropdown(label="Add a LoRA", info="Pick either high or low", choices=[], interactive=True)
                    ...
                    with gr.Column(scale=1):
                        with gr.Accordion("Generations", open=True, elem_classes=["themed-accordion", "vid-theme"]):
                            ...
                            # with gr.Row():
                            #     vid_gallery_dd = gr.Dropdown(label="Select Video Result", interactive=True, filterable=False)
                            # with gr.Row():
                            #     vid_prev_clip_btn = gr.Button("Select Previous", variant="secondary")
                            #     vid_next_clip_btn = gr.Button("Select Next", variant="secondary")
                            with gr.Accordion("Manage Files", open=False, elem_classes=["themed-accordion", "stop-theme"]):
            ...
            # def _update_seq_assets(pre, nid):
            ...
            #     inputs=[preview, selected_node, kf_generate_count, current_file_path, selected_node],
            #     outputs=[
            #         test_gen_image,
            ...
            test_vid_btn.click(
                ...
                inputs=[preview, selected_node, vid_generate_count, current_file_path, selected_node],
                outputs=[
                    test_vid_player,
            ...
            # Nav (Simpler wiring: Button -> Dropdown -> Change Event -> Image/JSON)
            ...
            # vid_gallery_dd.change(_eh_set_selected_video, inputs=[preview, selected_node, vid_gallery_dd], outputs=[preview, vid_player])
            ...
            # vid_gallery_dd.change(_eh_set_selected_video, inputs=[preview, selected_node, vid_gallery_dd], outputs=[vid_player])
            # vid_gallery_dd.change(
            #     lambda p, n, path: path if path else None,
            #     inputs=[preview, selected_node, vid_gallery_dd], 
            #     outputs=[vid_player]
            # )
            ...
            # vid_prev_clip_btn.click(_eh_prev_vid_clip, inputs=[preview, selected_node, vid_gallery_dd], outputs=[vid_gallery_dd])
            # vid_next_clip_btn.click(_eh_next_vid_clip, inputs=[preview, selected_node, vid_gallery_dd], outputs=[vid_gallery_dd])
            ...
            # seq_kf_batch_btn.click(handle_sequence_image_batch, inputs=[preview, selected_node, seq_kf_inputs["seq_kf_iter"]], outputs=[seq_run_status["status_window"]])
            # seq_ib_batch_btn.click(handle_sequence_video_batch, inputs=[preview, selected_node, seq_ib_inputs["seq_ib_iter"]], outputs=[seq_run_status["status_window"]])
            ...
            # seq_cascade_btn.click(cb_save_project, inputs=[current_file_path, preview, settings_json], outputs=[])
            ...
            # vid_load_outputs = [preview, vid_length, vid_prompt, vid_neg]
