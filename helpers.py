...
WORKFLOWS_DIR = (APP_ROOT / "workflows").resolve()
...
#     Generates the UI list for the sidebar.
#     Returns [(Label, ID_String), ...]
#     Now iterates explicit V2 structures.
#     """
#     data = _ensure_project(data)
#     rows: List[Tuple[str, str]] = []
#     default_dur = float(data["project"]["inbetween_generation"].get("duration_default_sec", 3.0))
...
#     seqs_dict = data.get("sequences", {})
#     sorted_seqs = sorted(seqs_dict.values(), key=lambda x: x.get("order", 0))
...
#             seq_total += _video_seconds(vid, default_dur)
            
#         setting_prompt = seq.get("setting_prompt", "").strip()
        
#         # Seq Label
#         # Use Sequence Index (calculated dynamically for display)
#         # Note: 'order' might be sparse or non-contiguous if we just deleted, 
#         # but here we just want a visual index.
#         si_label = f"{seq.get('order', 0) + 1:02d}" 
#         seq_label_text = f"{int(round(seq_total))} sec | {setting_prompt[:40]}" if setting_prompt else f"{int(round(seq_total))} sec"
#         rows.append((f"◆ s{si_label} {seq_label_text}", seq_id))
        
#         # Interleave Keyframes and Videos based on order
#         # We need to walk them logically. 
#         # Structure is usually: Vid(Open->KF1), KF1, Vid(KF1->KF2), KF2, Vid(KF2->Open)
...
#         # Map videos by start_id to easily find "next video"
#         videos_by_start = {}
#         for vid_id in seq.get("video_order", []):
#             vid = seq.get("videos", {}).get(vid_id)
#             if vid:
...
#         # 1. Start Video (Open -> First KF)
#         if "open" in videos_by_start:
#             vid = videos_by_start["open"]
#             dur = _video_seconds(vid, default_dur)
#             time_label = f"{_fmt_clock(current_time)}-{_fmt_clock(current_time+dur)}"
#             prompt = vid.get("inbetween_prompt", "").strip()
#             clip_label = f"{time_label} | {prompt[:40]}" if prompt else time_label
#             rows.append((f"▷ s{si_label} ib{vid_counter:02d} -- {clip_label}", vid["id"]))
#             current_time += dur
#             vid_counter += 1

#         # 2. Keyframes and following videos
#         for kf_id in seq.get("keyframe_order", []):
#             kf = seq.get("keyframes", {}).get(kf_id)
#             if not kf: continue
...
#             kf_text = " | ".join(parts) if parts else "Empty Keyframe"
#             rows.append((f"▣ s{si_label} kf{kf_counter:02d} -- {kf_text}", kf_id))
#             kf_counter += 1
            
#             # Following Video?
#             if kf_id in videos_by_start:
#                 vid = videos_by_start[kf_id]
#                 dur = _video_seconds(vid, default_dur)
#                 time_label = f"{_fmt_clock(current_time)}-{_fmt_clock(current_time+dur)}"
#                 prompt = vid.get("inbetween_prompt", "").strip()
#                 clip_label = f"{time_label} | {prompt[:40]}" if prompt else time_label
...
# ============================================================================
# PHASE 2A: CALLBACK FUNCTIONS - GUARANTEED TO RETURN DICT
# ============================================================================
# These three functions are the primary sources that write to preview_code.
# They are now GUARANTEED to return dict (never string).
# ============================================================================
...
def cb_save_as(save_as_name_or_path: str, settings_json: str, current_project: dict):
    ...
    # PHASE 2A: Input defensive check - will be removed in Phase 2D
    ...
    atomic_write(p, data)
    # _save_last_open_path_to_settings(str(p))  
    
    # PHASE 2A: GUARANTEED dict return
    return (data, str(p))
...
# def cb_load_last_project(settings_json_str: str):
#     try:
#         settings = json.loads(settings_json_str)
#         last_path = settings.get("last_open_project_path")
#         if last_path and Path(last_path).exists():
#             print(f"INFO: Reloading last open project: {last_path}")
#             # PHASE 2A: cb_open_file guaranteed to return dict
#             project_json, path = cb_open_file(last_path, settings_json_str)
#             return (project_json, path)
#     except Exception as e: print(f"Warning: Could not load last open project. {e}")
...
# ============================================================================
# END PHASE 2A CALLBACK FUNCTIONS
# ============================================================================
...
def cb_save_project(current_file_path: str, current_project: dict, settings_json: str):
    ...
    # PHASE 2A NOTE: This isinstance check is defensive during migration
    # if isinstance(current_project, str): current_project = _loads(current_project)
    ...
    try:
        ...
        # print(f"   Sequences in saved data: {list(data.get('sequences', {}).keys())}")
...
#     # Create backup of EXISTING file before overwriting (Safety)
#     if path.exists():
#         try:
...
#             # PHASE 1: Keep only last 10 backups
...
#                         print(f"Warning: Could not delete {old_backup.name}: {del_err}")
#         except Exception as e:
...
#         # If replace fails, we must notify and clean up
#         print(f"ERROR: Permission denied when saving to {path}. The file may be open in another program.")
#         try: os.remove(tmp_path)
#         except Exception: pass
#         # We do not return silently here; we let the UI/Console know the save failed essentially.
#         # But to prevent a crash, we stop here.
#         return
#     except Exception as e:
...
#     throttle_limit = settings.get("backups", {}).get("throttle_seconds", 60)
#     retention_limit = settings.get("backups", {}).get("retention_count", 10)
def atomic_write(path: Path, data: dict):
    global LAST_BACKUP_TIME
    import time
